				...
				for(var x=0;x<item.items.length;x++){
					...
					switch(_obj.type){
						...
						case 'radio':
                            ...
                            _html +='<input type="radio" style="height:15px;" class="'+_name+'" id="'+_name+'" name="'+_name+'"  '+_v+'><label class="mr20" for="'+_name+'" style="font-weight:normal;">'+_obj.text+'</label>'
							break;
						case 'checkbox':
							var _v = _obj.value===true?'checked':''
        ...
        bt.send('GetSoftList', 'ajax/GetSoftList', {}, function (res) {
            ...
            for (var i = 0; i < softs.length; i++)
            {
                ...
                for (var j = 0; j < softs[i].length; j++) {
                    ...
                    rec_box += '<li style="margin-bottom:5px"><span class="ico"><img src="/static/img/soft_ico/ico-' + info.name.toLowerCase() + '.png"></span><span class="name"><select class="sl-s-info  select_' + info.name + '" style="width:170px; height: 28px;">' + versions + '</select></span><span class="pull-right" >' + _install +'</span></li>';
                }
...
bt.soft = {
    ...
    update_soft: function (name, version, min_version, update_msg) {
        var _this = this;
...
bt.database = {
	...
	set_data_access:function(name){
		...
		bt.send('GetDatabaseAccess','database/GetDatabaseAccess',{name:name},function(rdata){
            ...
            var fdata = bt.data.database.data_access;
            fdata.title = '设置数据库【' + name + '】权限'
            var bs = bt.render_form(fdata);
			$('.name'+bs).val(name);
			setTimeout(function(){
	...
	input_sql:function(fileName,dataName){
        bt.confirm({ msg: '数据库【' + dataName+ '】将被覆盖，继续吗?',title:lan.database.input_title},function(index){
