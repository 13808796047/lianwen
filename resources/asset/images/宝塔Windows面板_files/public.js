		...
		$('.ace_header span').click(function (e) {
			...
			switch(type){
                ...
                case 'saveCaret':
                    ...
                    var file_store = $(".caret-list").data("file_store")
                    ...
                    var file_data = {
                        ...
                        area: '380px',
                        list: [
                            { name: 'path', disabled: true, width: '300px', value: editor_item.path },
		...
		$('.ace_catalogue_list').on('mousedown','.has-children .file_fold',function(e){
			...
			if(e.which === 3){
				...
				if($(this).attr('data-file') == 'Dir'){
                    ...
                    $('.ace_catalogue_menu li:eq(4)').hide();
				}else{
					$('.ace_catalogue_menu li:eq(0)').hide();
                    $('.ace_catalogue_menu li:eq(1)').hide();
                    $('.ace_catalogue_menu li:eq(4)').show();
				}
	...
	reader_file_dir_menu:function(obj,callback){
		...
		this.get_file_dir_list({p:obj.p,path:obj.path},function (res){
			...
			for(var i=0;i<_dir.length;i++){
                ...
                _dir_dom += '<li class="has-children children_' + obj.group + '" title="' + (obj.path + '/' + _data[0]) + '" data-filename="' + _data[0] +'" data-paths="'+ (obj.path+'/'+_data[0])+'" data-size="'+ (_data[1]) +'">\
			...
			for(var j=0;j<_files.length;j++){
                ...
                _files_dom += '<li class="has-children" title="' + (obj.path + '/' + _data[0]) + '" data-filename="' + _data[0]+'" data-paths="'+ (obj.path+'/'+_data[0])+'" data-size="'+ (_data[1]) +'" data-suffix="'+ _this.get_file_suffix(_data[0]) +'">\
			...
			if(res.PATH !== '/' && obj['group'] === 1){
                _html = '<li class="has-children upper_level" data-paths="' + _this.get_file_dir(res.PATH, 1) + '"><span>返回上级</span></li><li class="has-children" ><input type="text"  class="bt-input-text ace-file-search search-children-files" placeholder="搜索" style="width:100%;margin:0"></li>'
            ...
            $(".search-children-files").keyup(function () {
                var _search_txt = $(this).val()
                $("ul li.has-children:gt(1)").each(function (i, v) {
                    var _li = $(v)
                    if (_li.attr('data-filename').indexOf(_search_txt) >= 0) {
                        _li.show();
                    }
                    else {
                        _li.hide();
...
acme = {
    ...
    show_speed: function () {
        bt.send('get_lines', 'ajax/get_lines', {
            num: 10,
            filename: setup_path + "/panel/logs/letsencrypt.log"
        }, function (rdata) {
