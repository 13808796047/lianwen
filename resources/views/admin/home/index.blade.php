          ...
          @foreach($source_orders as $source=> $order)
            <tr>
              ...
              @switch(request()->date)
                ...
                @break
                @case('pre_month')
                <td>{{$order->count().'/'.\App\Models\Order::whereBetween('created_at',[\Carbon\Carbon::now()->subMonth()->startOfMonth(), \Carbon\Carbon::now()->subMonth()->endOfMonth()])->count()->groupBy($source)}}</td>
                @break
                @default
                <td>{{$order->count().'/'.\App\Models\Order::whereBetween('created_at',[\Carbon\Carbon::now()->startOfDay(), \Carbon\Carbon::now()->endOfDay()])->count()->groupBy($source)}}</td>
              @endswitch
