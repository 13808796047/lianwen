        ...
        <table class="table custom-data-table dataTable source">
          ...
          @foreach($source_orders as $source=> $order)
            <tr>
              ...
              @switch(request()->date)
                @case('yesterday')
                <td>{{$order->count().'/'.\App\Models\Order::whereBetween('created_at',[\Carbon\Carbon::now()->subDay()->startOfDay(), \Carbon\Carbon::now()->subDay()->endOfDay()])->where('form',$source)->count()}}</td>
                @break
                @case('month')
                <td>{{$order->count().'/'.\App\Models\Order::whereBetween('created_at',[\Carbon\Carbon::now()->startOfMonth(), \Carbon\Carbon::now()->endOfMonth()])->where('form',$source)->count()}}</td>
                @break
                @case('pre_month')
                <td>{{$order->count().'/'.\App\Models\Order::whereBetween('created_at',[\Carbon\Carbon::now()->subMonth()->startOfMonth(), \Carbon\Carbon::now()->subMonth()->endOfMonth()])->where('form',$source)->count()}}</td>
                @break
                @default
                <td>{{$order->count().'/'.\App\Models\Order::whereBetween('created_at',[\Carbon\Carbon::now()->startOfDay(), \Carbon\Carbon::now()->endOfDay()])->where('form',$source)->count()}}</td>
              @endswitch
